...
beforeAll(() => {
    ...
    // Patch: happy-dom v20+ is missing SyntaxError on the Window object,
    // which breaks querySelector/querySelectorAll with attribute selectors.
...
test("generates swap mode icons when iconOpen and iconClose are provided", () => {
    const data: AccordionData = {
        items: [
            {
                heading: "Test",
    ...
    const iconOpen = icon?.querySelector('[accordionary="icon-open"]');
    const iconClose = icon?.querySelector('[accordionary="icon-close"]');

    expect(iconOpen).not.toBeNull();
    expect(iconClose).not.toBeNull();
    expect(iconOpen?.innerHTML).toBe("−");
    expect(iconClose?.innerHTML).toBe("+");

    // Should not have direct text content (no rotate icon)
    expect(icon?.childElementCount).toBe(2);
});

test("generates swap mode icons with HTML content", () => {
    const data: AccordionData = {
        items: [
            {
                heading: "Test",
    ...
    const openSvg = '<svg class="open-icon"><path d="M0 0"/></svg>';
    const closeSvg = '<svg class="close-icon"><path d="M1 1"/></svg>';
    ...
    const iconOpen = element.querySelector('[accordionary="icon-open"]');
    const iconClose = element.querySelector('[accordionary="icon-close"]');

    expect(iconOpen?.innerHTML).toContain("open-icon");
    expect(iconClose?.innerHTML).toContain("close-icon");
});

test("uses rotate mode when only icon is provided (no swap)", () => {
    const data: AccordionData = {
        items: [
            {
                heading: "Test",
    ...
    // Should not have swap children
    const iconOpen = icon?.querySelector('[accordionary="icon-open"]');
    const iconClose = icon?.querySelector('[accordionary="icon-close"]');
    expect(iconOpen).toBeNull();
