...
import type { AccordionConfig, AccordionController, Item, ItemConfig, ItemController } from "./types";
 ...
 */
export function initAccordion(component: HTMLElement): AccordionController | null {
    ...
    if (openAttr && !["all", "first", "none"].includes(openAttr)) {
        warn(`Invalid accordionary-open="${openAttr}". Expected "all", "first", or "none". Defaulting to "none".`, component);
    }

    // Validate accordionary-multiple attribute
    if (multipleAttr && !["true", "false"].includes(multipleAttr)) {
        warn(`Invalid accordionary-multiple="${multipleAttr}". Expected "true" or "false". Defaulting to "true".`, component);
    }

    // Validate accordionary-link attribute
    if (linkAttr && !["true", "false"].includes(linkAttr)) {
        warn(`Invalid accordionary-link="${linkAttr}". Expected "true" or "false". Defaulting to "false".`, component);
    }
    ...
    if (speedAttr && (isNaN(parsedSpeed) || parsedSpeed < 0)) {
        warn(`Invalid accordionary-speed="${speedAttr}". Expected a positive number in milliseconds. Defaulting to 300.`, component);
    }

    const config: AccordionConfig = {
        openDefault: openAttr === "all" || openAttr === "first" ? openAttr : "none",
    ...
    if (itemElements.length === 0) {
        warn(`No items found. Add elements with accordionary="item" inside your component.`, component);
        return null;
    }
    ...
    for (const itemElement of itemElements) {
        ...
        const iconOpenElement = iconElement ? select("icon-open", iconElement) : null;
        const iconCloseElement = iconElement ? select("icon-close", iconElement) : null;

        // Validate required child elements
        if (!headingElement) {
            error(`Missing header element. Add accordionary="header" inside this item.`, itemElement);
            continue;
        }
        if (!contentElement) {
            error(`Missing content element. Add accordionary="content" inside this item.`, itemElement);
            continue;
        }
        if (!iconElement) {
            error(`Missing icon element. Add accordionary="icon" inside your header.`, itemElement);
            continue;
        }
        ...
        if (itemOpenAttr && !["true", "false"].includes(itemOpenAttr)) {
            warn(`Invalid accordionary-open="${itemOpenAttr}" on item. Expected "true" or "false".`, itemElement);
        }

        const itemConfig: ItemConfig = {
            openOverride: itemOpenAttr === "true" ? true : itemOpenAttr === "false" ? false : null,
