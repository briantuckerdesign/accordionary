 ...
 */
export function initItem(item: Item, accordionConfig: AccordionConfig, isFirst: boolean, allItems: Item[]) {
    const { headingElement, contentElement, iconElement, iconOpenElement, iconCloseElement, config } = item;

    // Determine icon mode: swap (has open/close children) vs rotate (default)
    const useSwapMode = iconOpenElement !== null && iconCloseElement !== null;
    ...
    const iconTransition = reduceMotion ? "none" : `transform ${speed}ms ${easing}`;
    ...
    if (useSwapMode) {
        // Swap mode: show/hide open and close icons
        iconOpenElement.style.display = isOpen ? "" : "none";
        iconCloseElement.style.display = isOpen ? "none" : "";
        if (config.disabled) {
            iconElement.style.visibility = "hidden";
        }
    } else {
        // Rotate mode: rotate a single icon
    ...
    function close() {
        ...
        if (useSwapMode) {
            iconOpenElement!.style.display = "none";
            iconCloseElement!.style.display = "";
        } else {
